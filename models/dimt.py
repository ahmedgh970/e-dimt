...
class DiMTBlock(nn.Module):
    """
    A DiMT block with adaptive layer norm zero (adaLN-Zero) conditioning.
    """
...
class DiMT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            DiMTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio) for _ in range(depth)
        ])
    ...
    def initialize_weights(self):
        ...
        # Zero-out adaLN modulation layers in DiMT blocks:
...
#                                   DiMT Configs                                  #
#################################################################################
def DiMT_XL_2(**kwargs):
    return DiMT(patch_size=2, hidden_size=1152, num_heads=16, depth=28, **kwargs)

def DiMT_L_2(**kwargs):
    return DiMT(patch_size=2, hidden_size=784, num_heads=16, depth=24, **kwargs)

def DiMT_B_2(**kwargs):
    return DiMT(patch_size=2, hidden_size=768, num_heads=12, depth=12,  **kwargs)

def DiMT_S_2(**kwargs):
    return DiMT(patch_size=2, hidden_size=384, num_heads=6, depth=12, **kwargs)
